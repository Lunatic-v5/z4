...
cek_reg_mode() {
    until [[ $MYIP =~ ^[0-9.]+$ && ${CLIENT_EXISTS} == '1' ]]; do
        ...
        CLIENT_EXISTS=$(wget -qO- https://raw.githubusercontent.com/Lunatic-v5/IZIN/Lunatic/ip | grep -w $MYIP | wc -l)
...
function download_config(){
    ...
    wget -O /tmp/ "${REPO}config/menu.zip" >/dev/null 2>&1
    mkdir /tmp/menu
    7z e  /tmp/ -o/tmp/menu/ >/dev/null 2>&1
    chmod +x /tmp/menu/*
